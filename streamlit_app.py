...
def app():
    ...
    **Experiment 2**  
    ...
    ④ 約30秒後に「ロボット行動計画」と「ロボットからの質問」が出力されるため、③の写真を見ながらロボットの質問に対して答えてください。質問された情報が写真にない場合は、"仮想の情報"を答えて構いません。自動で評価フォームが表示されるまで会話を続けてください。  
    ⑤ Critic モデルにより会話は自動終了し、評価フォームを記入 → 評価を保存 をクリック  
    ⑥ ① のモード選択をほかのものに選びなおしたら「会話をリセット」ボタンを押して再度実験（2パターン1回ずつ）  
    ...
    ④ 約30秒後に「ロボット行動計画」と「ロボットからの質問」が出力されるため、③の写真を見ながらロボットの質問に対して答えてください。質問された情報が写真にない場合は、"仮想の情報"を答えて構いません。自動で評価フォームが表示されるまで会話を続けてください。  
    ⑤ Critic モデルにより会話は自動終了し、評価フォームを記入 → 評価を保存 をクリック  
    ⑥ ① のモード選択をほかのものに選びなおしたら「会話をリセット」ボタンを押して再度実験（3パターン1回ずつ）  
